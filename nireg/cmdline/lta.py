...
def _build_parser() -> argparse.ArgumentParser:
    ...
    diff_p.add_argument('lta1', metavar='LTA1',
                        help='First (or only) LTA transform file.')
    diff_p.add_argument('lta2', metavar='LTA2', nargs='?', default=None,
                        help='Second LTA file.  Omit to compare LTA1 against identity.')
    diff_p.add_argument('--dist', type=int, default=2, choices=[1, 2, 3, 4, 5, 7],
                        metavar='{1,2,3,4,5,7}',
                        help='Distance type (default: 2).')
    diff_p.add_argument('--radius', type=float, default=100.0, metavar='MM',
                        help='Sphere / RMS radius in mm (dist 2 and 4, default: 100).')
    diff_p.add_argument('--normdiv', type=_positive_float, default=1.0, metavar='FLOAT',
                        help='Divide the final distance by this value (must be > 0, default: 1).')
    diff_p.add_argument('--invert1', action='store_true',
                        help='Invert the first transform before comparison.')
    diff_p.add_argument('--invert2', action='store_true',
