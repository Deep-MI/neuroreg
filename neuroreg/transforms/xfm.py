...
class XFM:
    ...
    def read(cls, filename: str | Path) -> XFM:
        ...
        ------
        ValueError
            If the file header is invalid or the linear transform block cannot
            be parsed.
        """
        path = Path(filename)
    ...
    def from_lta(cls, lta: LTA, comments: list[str] | None = None) -> XFM:
        ...
        comments : list[str] or None, optional
            Comment lines to preserve in the output wrapper. When omitted, a
            simple provenance comment is generated.

        Returns
        -------
        XFM
            Wrapper containing the same RAS-to-RAS matrix and inferred source /
            destination paths.
        """
    ...
    def to_lta(
        ...
        src_fname, dst_fname : str or None, optional
            Optional filenames to store in the output LTA metadata. When not
            provided, inferred paths from XFM comments are used if available.
